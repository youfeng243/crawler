...
class EEWorkerController(WorkerControllerWithInputThread):
    def __init__(self, proc=None, conf=None):
        self.conf = conf
        self.processor = None
        super(EEWorkerController, self).__init__(proc, conf)
    ...
    def reload_handler(self, packet, from_main_queue, to_main_queue):
        ...
        ret_data = self.processor.extract_obj.route.reload(packet.topic_id)
...
class EEWorker(BasicWorker):
    def __init__(self, conf=None, name=None):
        BasicWorker.__init__(self, conf, name, controller_class=EEWorkerController)
...
class EEWorkerPool(BasicWorkerPool):
    def __init__(self, init_count=BasicWorkerPool.DEFAULT_INIT_COUNT, conf=None):
        super(EEWorkerPool, self).__init__(init_count=init_count, conf=conf, tag='ee', worker_class=EEWorker)

    def reload_all(self, topic_id=-1):
        last_succ_ret = None
        for worker in self.subs:
            assert type(worker) == EEWorker
