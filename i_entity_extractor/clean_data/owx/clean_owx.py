...
class CleanOwx:
    def __init__(self):
        ...
        self.db = MongDb(mongo_conf['host'], mongo_conf['port'], mongo_conf['final_db'],
                         mongo_conf['username'],
                         mongo_conf['password'])
        self.sourceTable = 'enterprise_owing_tax'
        self.targetTable = 'enterprise_owing_tax'
    ...
    def do_clean(self,item):
        ...
        item["bulletin_date"] = self.deal_date(item.get("bulletin_date",""))
        item["owing_date"]    = self.deal_date(item.get("owing_date",""))
        item["ori_owing_date"] = self.deal_date(item.get("ori_owing_date", ""))
        url  = item.get("_src")[0]["url"]
        site = url.split("//")[-1].split("/")[0]
        if site == "www.gdltax.gov.cn":
            item["owing_tax_cur_period"] = 0.0
