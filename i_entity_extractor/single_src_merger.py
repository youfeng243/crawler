...
class SingleSourceMerger:
    ...
    def reload_merge_rule(self):
        ...
        put_to_dict_path(new_dict, [list], SingleSrcMergeRule.make_test_rule("*", "*", "*",
            '{"type":"append", "unique_keys":["_topo_token"], "sort_keys":[["_topo_token","topo_token"]]}'))
        put_to_dict_path(new_dict, ["*", "*", "_src"], SingleSrcMergeRule.make_test_rule("*", "*", "_src",
            '{"type":"append", "unique_keys":["site"], "sort_keys":[["_topo_token","topo_token"]]}'))
    ...
    def process(self, download_time, site, url, doc_fragment, topic_id, plugin_merge_funcs):
        ...
        if clear_existing:
            latest_merged_doc = {}
            new_meta = copy.deepcopy(SingleSourceMerger.initial_meta)
