...
from i_entity_extractor.extractors.judge_process.judge_process_extractor import JudgeProcessExtractor
from i_entity_extractor.entity_extractor_route import EntityExtractorRoute


class JudgeProcess(BaseEtl):
    def __init__(self, conf):
        BaseEtl.__init__(self,conf)
        route         = EntityExtractorRoute(conf)
        self.topic_id = 37
        topic_info    = route.all_topics.get(self.topic_id, None)
        self.judge_process_obj = JudgeProcessExtractor(topic_info, conf.get('log'))

    def process_data(self, item):
        etl_data = self.judge_process_obj.format_extract_data(item, self.topic_id)
        etl_data = self.judge_process_obj.after_process(etl_data)
        # etl_data.pop("_id")
    ...
    def etl(self):
        ...
        self.log.info("finish_judge_process_etl,process_num:%s\ttime_cost:%s" % (num, time.time() - begin_time))
