...
class SFExtractor(object):
    ...
    _public_time = re.compile(u'[0-9一二三四五六七八九零〇oO]{4}[年|\-|\.][0-9一二三四五六七八九十零〇oO]{1,3}[月|\-|\.][0-9一二三四五六七八九十零〇oO]{1,3}日?(?:\s*[0-9]{1,2}:[0-9]{1,2})?', re.I|re.S)
    ...
    _li = re.compile(r'<\s*li(.*?)>.*?</li>', re.I|re.S)
    # special cases
    ...
    # html
    _html = re.compile(r'<[\w\W]*html', re.I|re.S)
    # DTD
    ...
    def extract_title(self, text):
        ...
        new_title = title_arr[sort_list[0][0]]
    ...
    #利用正则提取日期
    def extract_publish_time(self, text):
        results = self._public_time.findall(text)
        return results
    ...
    def extract_content(self, text, thres):
        ...
        while i < len(blocks):
            ...
            if new_score > best_score:
                ...
                break
        """
        content = 'my_enter_special_char'.join(lines[best_block[0]:best_block[1]])
        content = self._multi.sub(' ', content) 
        content = content.replace(self._link_mark, '')
        content_list = content.split('my_enter_special_char')
        content_list = [cont.strip() for cont in content_list if len(cont.strip()) > 0]
        content = '\n'.join(content_list)
        return content, best_block, text
    ...
    def extract(self, raw_text, _thres=0):
        ...
        self.content, content_block, text = self.extract_content(text, _thres or self.min_block_len)
        texts = text.split('\n')#[:content_block[0]]
        lines = texts[:content_block[0]]
        #print "\n".join(lines)
        self.public_time = ""
        ...
        for line in lines[::-1]:
            try:
                t = self.extract_publish_time(line.decode('utf-8'))
                if t and dateparser.parse(t[0]) < cur_time:
                    self.public_time = t[0]
                    break
            except Exception as e:
                pass
        if not self.public_time:
            for line in texts[content_block[0]:content_block[-1]]:
                try:
                    t = self.extract_publish_time(line.decode('utf-8'))
                    if t and dateparser.parse(t[0] < cur_time):
                        self.public_time = t[0]
                    ...
                    pass
        return {"title":self.title, "public_time":self.public_time,"content":self.content, "desc":self.desc}
...
def test_file(p_in):
    ...
    for url in p_in:
        ...
        if raw_html:
            #print raw_html
            result = ext.extract(raw_html)
            print ext.extract_publish_time(raw_html)
...
if __name__ == '__main__':
    ...
    test_file("http://stock.eastmoney.com/news/1405,20161109682306590.html")
    if len(sys.argv)==2:
